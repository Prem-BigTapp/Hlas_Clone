...
def provide_intelligent_guidance(user_message: str, chat_history: list, session_id: str) -> str:
    ...
    try:
        ...
        if len(chat_history) <= 2:
            ...
            return "Great! I can help you with Insurance policies. Which would you like to learn about? ✈️🏠"
        else:
...
# from .maid_agent import run_maid_agent
# from .payment_agent import run_payment_agent
# # NEW imports for additional product agents
# from .car_agent import run_car_agent
# from .family_agent import run_family_agent
# from .early_agent import run_early_agent
# from .choice_agent import run_choice_agent
...
#             elif intent_result.product == Product.CAR:
#                 logger.info(f"LLM routing to CAR agent for session {session_id}")
#                 update_conversation_context(session_id, primary_product=Product.CAR, last_intent="product_inquiry")
#                 car_resp = run_car_agent(user_message, chat_history, session_id)
#                 return car_resp.get("output") if isinstance(car_resp, dict) else str(car_resp)

#             elif intent_result.product == Product.FAMILY:
#                 logger.info(f"LLM routing to FAMILY agent for session {session_id}")
#                 update_conversation_context(session_id, primary_product=Product.FAMILY, last_intent="product_inquiry")
#                 fam_resp = run_family_agent(user_message, chat_history, session_id)
#                 return fam_resp.get("output") if isinstance(fam_resp, dict) else str(fam_resp)

#             elif intent_result.product == Product.EARLY:
#                 logger.info(f"LLM routing to EARLY (Critical Illness) agent for session {session_id}")
#                 update_conversation_context(session_id, primary_product=Product.EARLY, last_intent="product_inquiry")
#                 early_resp = run_early_agent(user_message, chat_history, session_id)
#                 return early_resp.get("output") if isinstance(early_resp, dict) else str(early_resp)

#             elif intent_result.product == Product.CHOICE:
#                 logger.info(f"LLM routing to CHOICE agent for session {session_id}")
#                 update_conversation_context(session_id, primary_product=Product.CHOICE, last_intent="product_inquiry")
#                 choice_resp = run_choice_agent(user_message, chat_history, session_id)
#                 return choice_resp.get("output") if isinstance(choice_resp, dict) else str(choice_resp)
...
#                 return """I can help with that.\n\nWhich insurance?\n• Travel Insurance\n• Maid Insurance\n• Car Insurance\n• Family Protect\n• Early Protect (Critical illness)"""
...
#         ],
#         "off_topic": [
#             "I specialize in helping with insurance questions! 😊 What can I help you with regarding Travel, Maid, Car or Family insurance?",
#             "I'm here to assist with your insurance needs. How can I help with Travel, Maid, Car or Family insurance today? 🛡️",
#             "Let's talk insurance! I can help you with Travel, Maid, Car and Family insurance. What would you like to know? ✈️🏠🚗👪"
#         ],
#         "error": [
#             "Oops! Something went wrong on my end. 😅 Could you please try again?",
#             "I'm having a small technical hiccup. Please send your message again! 🔧",
...
#         # Enhanced agent_map - include all product agents
#         agent_map = {
#             Product.TRAVEL: run_travel_agent,
#             Product.MAID: run_maid_agent,
#             Product.CAR: run_car_agent,
#             Product.FAMILY: run_family_agent,
#             Product.EARLY: run_early_agent,
#             Product.CHOICE: run_choice_agent,
#         }
...
#                 # Call agent consistently passing session_id (agents should accept session_id if they use it)
#                 try:
#                     response_data = agent_function(user_message, chat_history, session_id)
#                 except TypeError:
#                     # fallback: some agents might not accept session_id param
#                     response_data = agent_function(user_message, chat_history)
...
#         else:
#             # If a RAG/informational intent, route to RAG by default
#             if intent == "informational":
#                 logger.info("Routing informational request to RAG agent (product unknown).")
#                 # product_str may be enum or string
#                 product_str = product.value if hasattr(product, "value") else str(product)
#                 return get_rag_response(user_message, chat_history, product_str)
