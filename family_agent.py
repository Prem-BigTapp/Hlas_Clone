...
class FamilyInfo(BaseModel):
    """Essential fields for Family Protect."""
    policy_start_date: Optional[str] = Field(None, description="YYYY-MM-DD")
    cep_customer: Optional[bool] = Field(None, description="Is user a CEP customer?")
    first_time_cep: Optional[bool] = Field(None, description="Is this first CEP purchase?")
    ...
    premium_payment_type: Optional[str] = Field(None, description="monthly or yearly")
    response: str = Field(..., description="Assistant's reply.")

def run_family_agent(user_message: str, chat_history: list, session_id: str):
    """
    Family Protect (Flow A): essentials only, step-by-step.
    """
    session = get_session(session_id)
    collected_info = session.get("collected_info", {}).get("family_info", {})
    ...
    chain = llm.with_structured_output(FamilyInfo, method="function_calling")
    today = datetime.now().strftime("%Y-%m-%d")

    prompt = [
        SystemMessage(content=f"""You are a family insurance assistant. Collect ONLY these essentials:
- policy_start_date (YYYY-MM-DD; must be today or later)
- cep_customer (true/false)
- first_time_cep (true/false)
- riders (true/false)
- spouse_coverage (true/false)
- children_coverage (true/false)
- premium_payment_type (monthly/yearly)

Rules:
- Today is {today}. Validate date.
- Normalize yes/no to booleans.
- Acknowledge and move to next missing item.
...
"""),
    ...
    result: FamilyInfo = chain.invoke(prompt)

    for k, v in result.model_dump().items():
        if k != "response" and v not in (None, "", []):
            collected_info[k] = v

    set_collected_info(session_id, "family_info", collected_info)
    logger.info(f"[Family] {session_id} collected={collected_info}")
    ...
    if all(k in collected_info and collected_info[k] not in (None, "", []) for k in required_info):
        ...
        try:
            ...
            logger.exception("Family recommendation error: %s", e)
            return ("I'm having trouble generating a family policy recommendation right now.\n"
